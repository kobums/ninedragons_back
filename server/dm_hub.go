...
// 달무티 대기 상태 마감 타이머 — 차례 방치는 자동 최소 플레이(없으면 패스)로
// 해소되고, 핸드 정산 화면은 짧은 연출 뒤 다음 핸드로 넘어간다
// (테스트에서 짧게 낮춘다).
var (
	dmTurnTimeout    = 30 * time.Second // playing — 자동 플레이/패스
	dmHandEndTimeout = 5 * time.Second  // hand_end — 순위 연출 후 다음 핸드
)

// dmRoom 게임(순수 상태)과 좌석별 연결의 매핑
type dmRoom struct {
	Game       *DMGame
	Clients    map[int]*DMClient // seat → client
	PhaseTimer *time.Timer       // 대기 상태 마감 타이머

	// DeadlineSeq 마지막으로 마감을 건 StateSeq — 같은 대기 상태에 스냅샷이
	// 여러 번 나가도 마감이 늘어나지 않게 하는 근거
	DeadlineSeq int
	...
	Spectators map[*DMClient]bool

	// LastReact 좌석별 마지막 리액션 발신 시각 (레이트리밋 장부)
	LastReact map[int]time.Time
}

// dmPhaseSignal 마감 타이머의 발화 표식 — 대기 상태 일련번호(AfkSeq)로
// 지나간 발화를 구분한다
type dmPhaseSignal struct {
	GameID string
	Seq    int
}
...
type DMHub struct {
	...
	privateLobbies map[string]*dmRoom
	...
	// 게임 메시지
	gameMessage chan DMGameMessage

	// 마감 타이머 발화 (time.AfterFunc → 허브 채널 경유)
	phaseFired chan dmPhaseSignal

	// 세션·유예 타이머 장부 (sessions/grace/graceExpired 필드 승격)
	sessionManager[*DMClient]

	// 셔플·선 결정용 난수원 (허브 고루틴에서만 사용)
	rng *rand.Rand
}

type DMGameMessage struct {
	Client  *DMClient
	Message DMMessage
}
...
func NewDMHub() *DMHub {
	return &DMHub{
		...
		rooms:          make(map[string]*dmRoom),
		privateLobbies: make(map[string]*dmRoom),
		activeCodes:    make(map[string]string),
		gameMessage:    make(chan DMGameMessage),
		phaseFired:     make(chan dmPhaseSignal, 8),
		sessionManager: newSessionManager[*DMClient](),
...
func (h *DMHub) Run() {
	for {
		...
		case client := <-h.register:
			...
			log.Printf("[DM] Client registered: %s", client.ID)
		...
		case client := <-h.unregister:
			if _, ok := h.clients[client]; ok {
				...
				log.Printf("[DM] Client unregistered: %s", client.ID)
			}
...
func (h *DMHub) handleGameMessage(gm DMGameMessage) {
...
func (h *DMHub) handleJoinGame(client *DMClient, msg DMMessage) {
	...
	log.Printf("[달무티][입장] game=%s | seat%d=%s 대기 입장 (%d/%d)",
		room.Game.ID, seat, displayName(client.Name), len(room.Game.Players), DMMaxPlayers)
	if room.Code == "" { // 사설 방 입장은 조용히 (공용 로비만 알림)
		notify("달무티 참가", fmt.Sprintf("%s 입장 (%d/%d)",
			displayName(client.Name), len(room.Game.Players), DMMaxPlayers))
	}

	h.sendToClient(client, DMMessage{
		Type: DMMsgPlayerJoined,
	...
	h.broadcastEvent(room, DMEventPayload{Kind: "joined", Seat: &seat, Name: client.Name,
...
func (h *DMHub) lobbyRoomFor(roomField string) *dmRoom {
	code := normalizeRoomCode(roomField)
	if code == "" {
		if h.lobby == nil {
			game := NewDMGame(uuid.New().String())
			h.lobby = &dmRoom{Game: game, Clients: map[int]*DMClient{}}
			h.rooms[game.ID] = h.lobby
			log.Printf("[DM] Created lobby game %s", game.ID)
		}
		return h.lobby
	}
	if code == roomCodeNew {
	...
	if room == nil {
		game := NewDMGame(uuid.New().String())
		room = &dmRoom{Game: game, Clients: map[int]*DMClient{}, Code: code}
		h.privateLobbies[code] = room
		h.rooms[game.ID] = room
		log.Printf("[DM] Created private room %s (code=%s)", game.ID, code)
	}
	return room
}
...
func (h *DMHub) isSpectator(client *DMClient) bool {
	if client.GameID == "" {
		return false
	}
...
func (h *DMHub) handleReact(client *DMClient, msg DMMessage) {
	room := h.rooms[client.GameID]
	...
	seat := client.Seat
	h.broadcastEvent(room, DMEventPayload{Kind: "react", Seat: &seat, Name: client.Name, Message: payload.Emoji})
}

// waitingRoomOf 클라이언트가 속한 시작 전 방 (공용 로비 또는 사설 방)
func (h *DMHub) waitingRoomOf(client *DMClient) *dmRoom {
	room := h.rooms[client.GameID]
...
func (h *DMHub) hostSeat(room *dmRoom) int {
	return hostSeatOf(room.Clients)
}

// dmHumanCount 방의 사람 수
func dmHumanCount(room *dmRoom) int {
	return humanCountOf(room.Clients)
}

// updateLobbyWaiting 로비 현황판 갱신 — 사람 1명 이상 대기 && 미시작.
// 사설 방은 현황판에 노출하지 않는다 (초대 링크로만 접근).
func (h *DMHub) updateLobbyWaiting(room *dmRoom) {
	if room.Code != "" {
		return
	}
	waiting := h.lobby != nil && h.lobby == room && !room.Game.Ready && dmHumanCount(room) >= 1
	lobbySetWaiting("dalmuti", waiting)
}

// handleFillBots host 가 빈 좌석을 연습봇으로 5인까지 채운 뒤 즉시
// 시작한다 (스폰은 join 을 거치지 않으므로 명시 시작 호출).
func (h *DMHub) handleFillBots(client *DMClient) {
	...
	for len(room.Game.Players) < DMFillBotTarget {
		botNo++
		if !h.spawnDMBot(room, fmt.Sprintf("%s%d", botName, botNo)) {
			break
		}
	}
...
func (h *DMHub) handleStart(client *DMClient) {
	room := h.waitingRoomOf(client)
	if room == nil {
		h.sendError(client, "대기실을 찾을 수 없습니다")
		return
	...
	if !room.Game.CanStart() {
		h.sendError(client, fmt.Sprintf("%d명 이상 모여야 시작할 수 있습니다", DMMinPlayers))
		return
	}
	h.startGame(room)
}

func (h *DMHub) startGame(room *dmRoom) {
	...
	log.Printf("[달무티][경기시작] game=%s | 인원=%d | 선=seat%d | %v",
		room.Game.ID, len(room.Game.Players), room.Game.CurrentSeat, names)
	if !dmRoomHasBot(room) { // 연습봇전은 운영자 알림을 억제한다
		notify("달무티 게임 시작", fmt.Sprintf("%d인전 시작", len(room.Game.Players)))
	}

	first := room.Game.CurrentSeat
	h.broadcastEvent(room, DMEventPayload{Kind: "game_started", Seat: &first,
		Name: room.Game.Players[first].Name,
		Message: fmt.Sprintf("게임 시작 — %s님이 첫 리드입니다 (%d핸드 총점제)",
			room.Game.Players[first].Name, DMHands)})
	h.afterProgress(room)
}

// removeFromLobby 대기실에서 좌석을 비우고 남은 좌석을 앞으로 당긴다
func (h *DMHub) removeFromLobby(room *dmRoom, client *DMClient) {
	oldSeat := client.Seat
	room.Game.RemovePlayer(oldSeat)
	delete(room.Clients, oldSeat)

	// 좌석 압축: oldSeat 뒤의 클라이언트를 한 칸씩 당긴다
	rebuilt := map[int]*DMClient{}
	...
	log.Printf("[달무티][퇴장] game=%s | %s 대기 퇴장 (%d/%d)",
		room.Game.ID, displayName(client.Name), len(room.Game.Players), DMMaxPlayers)

	// 사람이 아무도 없으면 방 자체를 정리한다 (남은 봇 러너도 종료)
	if dmHumanCount(room) == 0 {
		for _, c := range room.Clients {
			if c == nil {
				continue
			}
			h.sendToClient(c, DMMessage{Type: DMMsgSessionExpired})
			h.drop(c.SessionID)
		}
	...
	h.broadcastEvent(room, DMEventPayload{Kind: "left", Name: client.Name,
...
func (h *DMHub) roomOf(client *DMClient) *dmRoom {
	room := h.rooms[client.GameID]
	if room == nil || !room.Game.Ready {
		return nil
	}
...
func (h *DMHub) handlePlay(client *DMClient, msg DMMessage) {
	...
	payloadBytes, _ := json.Marshal(msg.Payload)
	var payload DMPlayPayload
	json.Unmarshal(payloadBytes, &payload)

	if err := room.Game.Play(client.Seat, payload.Cards); err != nil {
		h.sendError(client, err.Error())
		return
	}
	log.Printf("[달무티][제출] game=%s | seat%d=%s %v",
		room.Game.ID, client.Seat, displayName(client.Name), payload.Cards)
	h.afterProgress(room)
}

func (h *DMHub) handlePass(client *DMClient) {
	room := h.roomOf(client)
	if room == nil {
		h.sendError(client, "게임을 찾을 수 없습니다")
		return
...
func (h *DMHub) afterProgress(room *dmRoom) {
	h.drainEvents(room)
	if room.Game.Phase == DMPhaseGameOver {
		h.finishGame(room)
		return
	}
	h.syncDeadline(room)
...
// drainEvents 순수 규칙이 쌓은 이벤트를 dm_event 로 방송한다
func (h *DMHub) drainEvents(room *dmRoom) {
	for _, ev := range room.Game.DrainEvents() {
		payload := DMEventPayload{Kind: ev.Kind, Message: ev.Message}
...
func (h *DMHub) syncDeadline(room *dmRoom) {
	game := room.Game
	...
	dur := dmTurnTimeout
	if game.Phase == DMPhaseHandEnd {
		dur = dmHandEndTimeout
	}
...
func (h *DMHub) scheduleDeadline(room *dmRoom, dur time.Duration) {
	h.stopPhaseTimer(room)
	room.Game.AfkSeq++
	room.Game.Deadline = time.Now().Add(dur).UnixMilli()
	sig := dmPhaseSignal{GameID: room.Game.ID, Seq: room.Game.AfkSeq}
	room.PhaseTimer = time.AfterFunc(dur, func() {
		h.phaseFired <- sig
	})
}

func (h *DMHub) stopPhaseTimer(room *dmRoom) {
	stopTimer(&room.PhaseTimer)
}

// handlePhaseFired 마감 발화 — 단계별 자동 행동으로 해소한다.
//   - playing: 자동 최소 플레이 (낼 수 없으면 패스)
//   - hand_end: 다음 핸드 진행 (3핸드를 다 쳤으면 종료)
func (h *DMHub) handlePhaseFired(sig dmPhaseSignal) {
	...
	case DMPhasePlaying:
		...
		h.broadcastEvent(room, DMEventPayload{Kind: "afk", Seat: &seat, Name: actor.Name,
			Message: fmt.Sprintf("%s님이 오래 응답하지 않아 자동으로 진행합니다", actor.Name)})
		if cards := game.AutoPlay(seat); cards != nil {
			game.Play(seat, cards)
			log.Printf("[달무티][자동진행] game=%s | seat%d 무응답 — 자동 제출 %v",
				game.ID, seat, cards)
		} else {
			game.Pass(seat)
			log.Printf("[달무티][자동진행] game=%s | seat%d 무응답 — 자동 패스", game.ID, seat)
		}

	case DMPhaseHandEnd:
		log.Printf("[달무티][핸드전환] game=%s | %d번째 핸드 마감", game.ID, game.HandNo)
		game.AdvanceHand(h.rng)
...
// buildDMState 개인화 게임 스냅샷. 재접속 복원과 일반 갱신이 같은 경로를
// 쓴다. 은닉: yourHand 는 본인에게만 실린다 — 빈 손이어도 빈 배열 []이고,
// 타인·관전자(viewerSeat -1)의 raw JSON 에는 필드 자체가 없다. 남은 장수
// (handCount)와 테이블 세트(tableSet)만 공용 정보다.
func (h *DMHub) buildDMState(room *dmRoom, viewerSeat int) DMGameStatePayload {
	game := room.Game
	...
	for _, p := range game.Players {
		...
		players = append(players, DMPlayerView{
			Seat:      p.Seat,
			Name:      p.Name,
	...
	endsAt := int64(0)
	if game.Phase == DMPhasePlaying || game.Phase == DMPhaseHandEnd {
		endsAt = game.Deadline
	}

	return DMGameStatePayload{
		GameID:      game.ID,
		RoomCode:    room.Code,
		Phase:       game.Phase,
...
func (h *DMHub) broadcastState(room *dmRoom) {
	for seat, c := range room.Clients {
		if c == nil {
			continue
		}
		h.sendToClient(c, DMMessage{
			Type:    DMMsgGameState,
			Payload: h.buildDMState(room, seat),
		})
	}
	if len(room.Spectators) > 0 {
		msg := DMMessage{Type: DMMsgGameState, Payload: h.buildDMState(room, -1)}
...
func (h *DMHub) broadcastEvent(room *dmRoom, event DMEventPayload) {
	h.broadcastToRoom(room, DMMessage{Type: DMMsgEvent, Payload: event})
}

// finishGame 게임 종료 발표·전적 기록·방 정리 (재대결은 같은 방 코드
// 재입장으로 — finishGame 이 코드를 반납해 같은 코드로 새 방을 만들 수 있다)
func (h *DMHub) finishGame(room *dmRoom) {
	game := room.Game
	h.stopPhaseTimer(room)
	game.Deadline = 0
	...
	message := fmt.Sprintf("%s님이 %d핸드 총점 1위로 승리했습니다!",
		strings.Join(winnerNames, " · "), DMHands)
	reason := "top_points"
	if len(winnerNames) > 1 {
		message = fmt.Sprintf("동점 공동 우승 — %s", strings.Join(winnerNames, " · "))
		reason = "tie_points"
	}
	...
	h.broadcastEvent(room, DMEventPayload{Kind: "game_over", Seat: &first,
		Name: strings.Join(winnerNames, " · "), Message: message})
	// 최종 스냅샷을 먼저 보낸 뒤 종료를 알린다
	// (봇 러너는 dm_game_over 를 보고 스스로 끝난다)
	h.broadcastState(room)
	h.broadcastToRoom(room, DMMessage{
		Type: DMMsgGameOver,
		Payload: DMGameOverPayload{
			WinnerSeats: append([]int{}, game.WinnerSeats...),
			WinnerNames: append([]string{}, winnerNames...),
			Players:     h.buildDMState(room, -1).Players,
			Message:     message,
		},
	})

	log.Printf("[달무티][경기결과] game=%s | 승자=%v(%s) | 소요=%s",
		game.ID, game.WinnerSeats, displayName(strings.Join(winnerNames, " · ")),
		matchDuration(game.StartedAt))

	winner := ""
	if len(winnerNames) == 1 {
		winner = displayName(winnerNames[0])
	}
	RecordMatch(MatchRecord{
		Game:     "dalmuti",
		Players:  strings.Join(names, " vs "),
		Winner:   winner, // 공동 우승은 무승부 취급 ("")
		Reason:   reason,
		Duration: matchSeconds(game.StartedAt),
		Bot:      dmRoomHasBot(room),
	})
...
func (h *DMHub) handleDisconnect(client *DMClient) {
	...
	log.Printf("[달무티][연결끊김] game=%s | seat%d=%s 재접속 대기 시작 (%.0f초)",
		room.Game.ID, client.Seat, displayName(client.Name), h.grace.Seconds())

	h.broadcastToRoom(room, DMMessage{
		Type: DMMsgPlayerDisconnected,
		Payload: DMPlayerDisconnectedPayload{
			Seat:         client.Seat,
			Name:         client.Name,
...
func (h *DMHub) handleGraceExpired(sessionID string) {
	...
	room := h.rooms[client.GameID]
	if room == nil || room.Game.Phase == DMPhaseGameOver || !room.Game.Ready {
		return
	}

	seat := client.Seat
	log.Printf("[달무티][봇대체] game=%s | seat%d=%s 유예 만료 → 연습봇 대체",
		room.Game.ID, seat, displayName(client.Name))

	bot := h.takeoverDMBot(room, seat, client.Name)
	room.Clients[seat] = bot
	h.sessions[bot.SessionID] = bot

	h.broadcastEvent(room, DMEventPayload{Kind: "bot_takeover", Seat: &seat,
		Name:    client.Name,
		Message: fmt.Sprintf("%s님 자리를 봇이 이어받았습니다", client.Name)})
	// 새 봇이 이 스냅샷을 받아 차례가 남았으면 즉시 이어간다
	h.broadcastState(room)
}

// handleRejoin 세션 ID로 기존 게임에 재접속
func (h *DMHub) handleRejoin(client *DMClient, msg DMMessage) {
	payloadBytes, _ := json.Marshal(msg.Payload)
	var payload DMRejoinPayload
	json.Unmarshal(payloadBytes, &payload)

	old := h.sessions[payload.SessionID]
	if old == nil || old.Bot {
		h.sendToClient(client, DMMessage{Type: DMMsgSessionExpired})
		return
	}
	...
	log.Printf("[달무티][재접속] game=%s | seat%d=%s 재접속 완료",
		room.Game.ID, client.Seat, displayName(client.Name))

	h.broadcastToRoom(room, DMMessage{
		Type:    DMMsgPlayerReconnected,
		Payload: DMPlayerReconnectedPayload{Seat: client.Seat, Name: client.Name},
	})
...
func (h *DMHub) clearGameSessions(room *dmRoom) {
	clearRoomSessions(&h.sessionManager, room.Clients)
}

// ==================== 전송 ====================

func (h *DMHub) sendError(client *DMClient, message string) {
	h.sendToClient(client, DMMessage{Type: DMMsgError, Payload: DMErrorPayload{Message: message}})
}

func (h *DMHub) sendToClient(client *DMClient, message DMMessage) {
	if client == nil {
		return
	}
	sendTo(&client.wsClient, message, "[DM] ")
}

func (h *DMHub) broadcastToRoom(room *dmRoom, message DMMessage) {
	for _, c := range room.Clients {
		if c != nil {
...
func ServeDMWs(hub *DMHub, w http.ResponseWriter, r *http.Request) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Println("[DM] Error upgrading connection:", err)
		return
	}
	...
	go readLoop(conn, "[DM] ",
		func(msg DMMessage) { hub.gameMessage <- DMGameMessage{Client: client, Message: msg} },
