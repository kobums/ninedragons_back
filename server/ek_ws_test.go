...
// 테스트에서는 대기 상태 마감을 짧게 낮춘다 (자동 뽑기·안돼 창 통과·
// 무작위 호의·무작위 되꽂기)
func init() {
...
// 가장 중요한 회귀 장치 (안돼 창 교착·차례 미전환·종료 판정 감지).
...
// TestEKBotQuality 4봇 30판의 완주율·평균 소요 차례 수 — 무한 안돼 루프나
// 교착이 없는지를 숫자로 고정한다.
...
func TestEKHiddenState(t *testing.T) {
	...
	if game.Phase != EKPhaseNopeWindow {
		t.Fatalf("미리보기를 냈는데 phase=%s", game.Phase)
	}
...
func TestEKNopeWindowStateMachine(t *testing.T) {
	...
	if game.Deadline <= 0 {
		t.Fatal("안돼 창 스냅샷의 endsAt(Deadline) 부재")
	}
	...
	// 마지막 안돼 좌석(seat1)을 뺀 전원 통과 → 짝수 겹이므로 건너뛰기 발동
	...
	// 창이 닫힌 뒤의 뒤늦은 안돼·통과는 조용히 무시된다 (에러가 아니다)
	ekDrain(t, clients[0])
