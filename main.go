...
func main() {
	...
	cpHub := server.NewCPHub()
	nmHub := server.NewNMHub()
	ccHub := server.NewCCHub()
	...
	ekHub := server.NewEKHub()
	miHub := server.NewMIHub()
	rfHub := server.NewRFHub()
	sbHub := server.NewSBHub()
	slHub := server.NewSLHub()
	azHub := server.NewAZHub()
	ctHub := server.NewCTHub()
	suHub := server.NewSUHub()
	bgHub := server.NewBGHub()
	rrHub := server.NewRRHub()
	ruHub := server.NewRUHub()
	bzHub := server.NewBZHub()
	...
	endpoints := []struct {
		...
		{"Coup", "/ws/coup", cpHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeCPWs(cpHub, w, r) }},
		{"6 Nimmt", "/ws/nimmt", nmHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeNMWs(nmHub, w, r) }},
		{"Ciao Ciao", "/ws/ciaociao", ccHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeCCWs(ccHub, w, r) }},
		{"Cockroach Poker", "/ws/cockroach", crHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeCRWs(crHub, w, r) }},
		{"Insider", "/ws/insider", idHub.Run,
		...
		{"Skull King", "/ws/skullking", kgHub.Run,
		...
		{"Exploding Kittens", "/ws/kittens", ekHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeEKWs(ekHub, w, r) }},
		{"The Mind", "/ws/mind", miHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeMIWs(miHub, w, r) }},
		{"Coup Reformation", "/ws/reformation", rfHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeRFWs(rfHub, w, r) }},
		{"Saboteur", "/ws/saboteur", sbHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeSBWs(sbHub, w, r) }},
		{"Splendor", "/ws/splendor", slHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeSLWs(slHub, w, r) }},
		{"Azul", "/ws/azul", azHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeAZWs(azHub, w, r) }},
		{"Citadels", "/ws/citadels", ctHub.Run,
		...
		{"Rummikub", "/ws/rummikub", ruHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeRUWs(ruHub, w, r) }},
		{"Bohnanza", "/ws/bohnanza", bzHub.Run,
			func(w http.ResponseWriter, r *http.Request) { server.ServeBZWs(bzHub, w, r) }},
	}
